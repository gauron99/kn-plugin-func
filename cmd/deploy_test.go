...
runtime: go`,
		...
		{
			name:      "ns in func.yaml, given via cli (always override), expect override",
			namespace: "test-ns-deploy",
			expectNS:  "test-ns-deploy",
			registry:  "docker.io/4141gauron3268",
			funcFile: `name: test-func
...
runtime: go`,
		...
		{
			name:     "ns in func.yaml, not given via cli, current ns does NOT match func.yaml, expect warning",
			registry: "docker.io/4141gauron3268",
...
runtime: go`,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// ns taken from mock kubeconfig (test-ns-deploy)
			mockCurrentNamespace, err := k8s.GetNamespace("")
			if err != nil {
				t.Fatalf("GetNamespace err: %s", err)
			}
			...
			if fileFunction.Namespace != tt.expectNS {
				t.Fatalf("Expected namespace '%s' but function has '%s' namespace", mockCurrentNamespace, fileFunction.Namespace)
			}
		})
	}
}
